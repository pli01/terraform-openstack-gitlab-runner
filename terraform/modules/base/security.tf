...
# runner
resource "openstack_networking_secgroup_v2" "runner_secgroup_1" {
  name        = format("%s-%s", var.prefix_name, "runner_secgroup_1")
  description = "runner security group"
  #delete_default_rules = true
}

resource "openstack_networking_secgroup_rule_v2" "ssh_worker_runner_secgroup_rule_1" {
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 22
  port_range_max    = 22
  remote_ip_prefix  = var.network_worker["cidr"]
  security_group_id = openstack_networking_secgroup_v2.runner_secgroup_1.id
}
